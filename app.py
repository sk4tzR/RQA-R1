...
def display_result(result):
    ...
    # Вычисляем текстовую оценку
    if result['disagreement'] < 0.1:
        confidence_text = "✅ Высокая"
        help_text = "Модель уверена в результате. Обе головы классификатора согласованы."
    ...
    # Одна компактная метрика с числом и оценкой
    st.metric(
        "🎯 Уверенность анализа",
        f"{confidence_text} ({result['disagreement']:.3f})",
        help=help_text
    )
