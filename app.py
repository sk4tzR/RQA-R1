...
def display_result(result):
    ...
    # Вычисляем текстовую оценку на основе disagreement
    if result['disagreement'] < 0.1:
        confidence_text = "✅ Высокая"
        confidence_color = "green"
        help_text = "Модель уверена в результате анализа. Обе головы классификатора согласованы."
    elif result['disagreement'] < 0.3:
        confidence_text = "⚖️ Средняя"
        confidence_color = "orange"
        help_text = "Небольшие расхождения в оценке модели, но результат надёжен."
    else:
        confidence_text = "⚠️ Низкая"
        confidence_color = "red"
        help_text = "Текст может быть сложным для интерпретации. Рекомендуется критически оценить результат."
    
    # Отображаем метрику с числом и текстовой оценкой
    col1, col2 = st.columns([1, 2])
    with col1:
        st.metric("📊 Disagreement", f"{result['disagreement']:.3f}")
    with col2:
        st.markdown(f"**Уверенность:** :{confidence_color}[{confidence_text}]")
